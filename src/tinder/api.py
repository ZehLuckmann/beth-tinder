import requests
import threading
import tinder.constants as constants
import tinder.errors as errors
...
class TinderAPI(object):
    ...
    def fb_friends(self):
        """
        Busca todos os amigos do Facebook que estão usando o Tinder
        Retorna um objeto contendo uma matriz com todos os listados.
        """
        return self._get("/group/friends")

    def like_message(self, message):
        """
        Like em uma mensagem enviada para você
        """
        return self._post("/message/{}/like".format(message.id))

    def unlike_message(self, message):
        """
        Remove o like de uma mensagem
        """
